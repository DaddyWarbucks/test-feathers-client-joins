        ...
        <div className="card mb-4 card-light">
          ...
          <div className={`card-body ${docsOpen ? '' : 'd-none'}`}>
            <p>
              ...
              on the client vs on the server. It is also a test bed and exmaple
              of a new v2 for{' '}
              <a
                href="https://github.com/feathersjs-ecosystem/batch-loader/tree/v2"
                target="_blank"
            ...
            <p>
              ...
              <code>app.service('api/posts').find()</code>. The posts then join
              a <code>user</code> and <code>comments</code>. Each user then
              joins on a <code>bio</code>, and each comment subsequently joins
              on its <code>user</code> (and subsequently its bio).
            </p>
            <ul>
              <li className="mb-2">
            ...
            <ul>
              <li className="mb-2">
                ...
                actual HTTP requests. The service was called, but it did not
                make a socket/rest request. It simply created "intructions" for
                feathers-batch.
              </li>
              <li className="mb-2">
                The app uses authentication. This means that there are
              ...
              <li className="mb-2">
                For simplicty in this example, the app uses NeDB as its
                database. NeDB is a filesystem database where the records are
                just flat JSON files. But, its important to note that it is not
                making an HTTP request to some database server that lives on
                another machine. As impressive as the different loading
                techniques are, it should be noted that on a production database
                there would be a corresponding number of HTTP requests (think
